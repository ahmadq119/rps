...
if ($request == 'caridata') {
    ...
    $sql = "SELECT * FROM View_penerimaan_sample_labsensori
...
if ($request == "update") {
    ...
    try {
        ...
        $check_sql = "SELECT COUNT(*) AS jumlah FROM sample_labsensori WHERE kd_sample = ?";
        ...
        if ($row['jumlah'] > 0) {
            ...
            $sql = "INSERT INTO sample_labsensori (kd_sample, tgl_uji, metode_pengujian, sts_terima) 
            VALUES (?, ?, ?, ?)";
        ...
        $query = "SELECT *
        FROM View_penerimaan_sample_labsensori
        WHERE kd_sample = ?";
