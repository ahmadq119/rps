...
class Html2Pdf
{
    ...
    protected $_hideFooter = []; // array : list of pages which the footer gonna be hidden
    ...
    protected static $_tables = []; // static table to prepare the nested html tables
    ...
    public function output($name = 'document.pdf', $dest = 'I')
    {
        ...
        if (!in_array($dest, ['I', 'D', 'F', 'S', 'FI', 'FD', 'E'])) {
            throw new Html2PdfException(
                'The output destination mode [' . $dest . '] is invalid'
            );
        }
        ...
        if ($dest !== 'S') {
            ...
            if (strtolower(substr($name, -4)) !== '.pdf') {
                throw new Html2PdfException(
                    'The output document name [' . $name . '] is not a PDF name'
                );
            }
        }
...
<html>
    ...
    protected function setDefaultMargins($margins)
    {
        ...
        $this->_defaultLeft = $this->cssConverter->convertToMM(
            $margins[0] . 'mm'
        );
        $this->_defaultTop = $this->cssConverter->convertToMM(
            $margins[1] . 'mm'
        );
        $this->_defaultRight = $this->cssConverter->convertToMM(
            $margins[2] . 'mm'
        );
        $this->_defaultBottom = $this->cssConverter->convertToMM(
            $margins[3] . 'mm'
        );
    }
    ...
    protected function _setNewPage(
        ...
        $this->_background =
            $background !== null ? $background : $this->_background;
        $this->_maxY = 0;
        $this->_maxX = 0;
        $this->_maxH = 0;
        $this->_maxE = 0;
        ...
        if (!$this->_subPart && !$this->_isSubPart) {
            if (is_array($this->_background)) {
                ...
                if (
                    isset($this->_background['img']) &&
                    is_array($this->_background['img'])
                ) {
                    $imageWidth = $this->cssConverter->convertToMM(
                        $this->_background['width'],
                        $this->pdf->getW()
                    );
                    $imageHeight =
                        ($imageWidth * $this->_background['img']['height']) /
                        $this->_background['img']['width'];

                    $posX = $this->cssConverter->convertToMM(
                        $this->_background['posX'],
                        $this->pdf->getW() - $imageWidth
                    );
                    $posY = $this->cssConverter->convertToMM(
                        $this->_background['posY'],
                        $this->pdf->getH() - $imageHeight
                    );

                    $this->pdf->Image(
                        $this->_background['img']['file'],
                        $posX,
                        $posY,
                        $imageWidth
                    );
    ...
    protected function setMargins()
    {
        ...
        $this->_margeLeft =
            $this->_defaultLeft +
            (isset($this->_background['left'])
                ? $this->_background['left']
                : 0);
        $this->_margeRight =
            $this->_defaultRight +
            (isset($this->_background['right'])
                ? $this->_background['right']
                : 0);
        $this->_margeTop =
            $this->_defaultTop +
            (isset($this->_background['top']) ? $this->_background['top'] : 0);
        $this->_margeBottom =
            $this->_defaultBottom +
            (isset($this->_background['bottom'])
                ? $this->_background['bottom']
                : 0);
    ...
    protected function _setNewPositionForNewLine($curr = null)
    {
        ...
        if (
            $curr !== null &&
            $sub->parsingHtml->code[$this->_parsePos]->getName() === 'write'
        ) {
            $txt = $sub->parsingHtml->code[$this->_parsePos]->getParam('txt');
            $txt = str_replace(
                '[[page_cu]]',
                $sub->pdf->getMyNumPage($this->_page),
                $txt
            );
            $sub->parsingHtml->code[$this->_parsePos]->setParam(
                'txt',
                substr($txt, $curr + 1)
            );
        } else {
    ...
    protected function createSubHTML()
    {
        ...
        if ($this->parsingCss->value['width']) {
            $marge =
                ...
                $this->parsingCss->value['padding']['r'];
            $marge +=
                $this->parsingCss->value['border']['l']['width'] +
                $this->parsingCss->value['border']['r']['width'];
            $marge =
                $this->pdf->getW() - $this->parsingCss->value['width'] + $marge;
        } else {
     ...
     * @param  string $img
     */
    protected function _listeAddLevel(
        $type = 'ul',
        $style = '',
        $img = null,
        $start = null
        ...
        // add the new level
        $this->_defList[count($this->_defList)] = [
            'style' => $style,
            'nb' => $start,
            'img' => $img,
        ];
    ...
    protected function _makeHTMLcode()
    {
        ...
        for (
            ...
            if (
                in_array($action->getName(), ['table', 'ul', 'ol']) &&
                !$action->isClose()
            ) {
                ...
                while (
                    isset($this->parsingHtml->code[$this->_tempPos]) &&
                    !(
                        $this->parsingHtml->code[$this->_tempPos]->getName() ==
                            $tagOpen &&
                        $this->parsingHtml->code[$this->_tempPos]->isClose()
                    )
                ) {
                    // make the action
                    $this->_executeAction(
                        $this->parsingHtml->code[$this->_tempPos]
                    );
                    $this->_tempPos++;
                }
    ...
    protected function _drawImage($src, $subLi = false)
    {
        ...
        if (strpos($src, 'data:') === 0) {
            $src = base64_decode(
                preg_replace('#^data:image/[^;]+;base64,#', '', $src)
            );
        ...
        if (!is_array($infos) || count($infos) < 2) {
            if ($this->_testIsImage) {
                $e = new ImageException(
                    'Unable to get the size of the image [' . $src . ']'
                );
                $e->setImage($src);
                throw $e;
            }
            ...
            if ($this->_fallbackImage) {
                ...
                if (count($infos) < 2) {
                    $e = new ImageException(
                        'Unable to get the size of the fallback image [' .
                            $src .
                            ']'
                    );
                    $e->setImage($src);
        ...
        if (
            isset($this->parsingCss->value['max-width']) &&
            $this->parsingCss->value['max-width'] < $w
        ) {
        ...
        if (
            isset($this->parsingCss->value['max-height']) &&
            $this->parsingCss->value['max-height'] < $h
        ) {
        ...
        if ($float && $this->_maxH && !$this->_tag_open_BR([])) {
            return false;
        }
        ...
        if (
            !$float &&
            $x + $w > $this->pdf->getW() - $this->pdf->getrMargin() &&
            $this->_maxH
        ) {
        ...
        $hT = 0.8 * $this->parsingCss->value['font-size'];
        if ($subLi && $h < $hT) {
            $y += $hT - $h;
        }
        ...
        if ($parentWidth > $w && $float !== 'left') {
            if (
                ...
                $x =
                    ...
                    $parentWidth -
                    $w -
                    $this->parsingCss->value['margin']['r'] -
        ...
        $h +=
            ...
            $this->parsingCss->value['margin']['b'];
     ...
     * @return boolean
     */
    protected function _drawRectangle(
        $x,
        $y,
        $w,
        $h,
        ...
        if ($background['image']) {
            ...
            if (!is_array($imageInfos) || count($imageInfos) < 2) {
                if ($this->_testIsImage) {
                    $e = new ImageException(
                        'Unable to get the size of the image [' . $iName . ']'
                    );
                    $e->setImage($iName);
                    throw $e;
                }
                ...
                $imageWidth =
                    ((72. / 96.) * $imageInfos[0]) / $this->pdf->getK();
                $imageHeight =
                    ((72. / 96.) * $imageInfos[1]) / $this->pdf->getK();

                // prepare the position of the backgroung
                if ($iRepeat[0]) {
                    $iPosition[0] = $bX;
                } elseif (
                    preg_match('/^([-]?[0-9\.]+)%/isU', $iPosition[0], $match)
                ) {
                    $iPosition[0] =
                        $bX + ($match[1] * ($bW - $imageWidth)) / 100;
                } else {
                ...
                if ($iRepeat[1]) {
                    ...
                    preg_match('/^([-]?[0-9\.]+)%/isU', $iPosition[1], $match)
                ) {
                    $iPosition[1] =
                        $bY + ($match[1] * ($bH - $imageHeight)) / 100;
                } else {
            ...
            if (!$found) {
                for ($y = 0; $y < $amountCorr; $y++) {
                    for (
                        ...
                        if (
                            isset($corr[$y][$xPrevious]) &&
                            is_array($corr[$y][$xPrevious]) &&
                            $corr[$y][$xPrevious][2] > $previousCell
                        ) {
                            $m = max(
                                $m,
                                $cases[$corr[$y][$xPrevious][1]][
                                    $corr[$y][$xPrevious][0]
                                ]['w'] / $corr[$y][$xPrevious][2]
                            );
                            break;
                        }
                    }
                ...
                if (isset($corr[$y][$x]) && is_array($corr[$y][$x])) {
                    ...
                    if ($corr[$y][$x][3] == 1) {
                        ...
                        for ($j = 1; $j < $corr[$y][$x][3]; $j++) {
                            ...
                            if (isset($corr[$ty][$tx])) {
                                $cases[$corr[$ty][$tx][1]][$corr[$ty][$tx][0]][
                                    'dw'
                                ] +=
    ...
    protected function _tag_open_PAGE($param)
    {
        ...
        $resetPageNumber =
            isset($param['pagegroup']) && $param['pagegroup'] === 'new';

        if (
            array_key_exists('hideheader', $param) &&
            $param['hideheader'] !== 'false' &&
            !empty($param['hideheader'])
        ) {
            $this->_hideHeader = (array) array_merge(
                $this->_hideHeader,
                explode(',', $param['hideheader'])
            );
        }

        if (
            array_key_exists('hidefooter', $param) &&
            $param['hidefooter'] !== 'false' &&
            !empty($param['hidefooter'])
        ) {
            $this->_hideFooter = (array) array_merge(
                $this->_hideFooter,
                explode(',', $param['hidefooter'])
            );
        }
        ...
        if ($newPageSet) {
            ...
            if (isset($param['format'])) {
                ...
                if (preg_match('/^([0-9]+)x([0-9]+)$/isU', $format, $match)) {
                    $format = [(int) $match[1], (int) $match[2]];
                }
            }
            ...
            if (isset($param['backimg'])) {
                ...
                $background['posX'] = isset($param['backimgx'])
                    ? $param['backimgx']
                    : 'center'; // horizontal position of the image
                $background['posY'] = isset($param['backimgy'])
                    ? $param['backimgy']
                    : 'middle'; // vertical position of the image
                $background['width'] = isset($param['backimgw'])
                    ? $param['backimgw']
                    : '100%'; // width of the image (100% = page width)

                // convert the src of the image, if parameters
                $background['img'] = str_replace(
                    '&amp;',
                    '&',
                    $background['img']
                );
            ...
            // convert to mm
            $background['top'] = $this->cssConverter->convertToMM(
                $background['top'],
                $this->pdf->getH()
            );
            $background['bottom'] = $this->cssConverter->convertToMM(
                $background['bottom'],
                $this->pdf->getH()
            );
            $background['left'] = $this->cssConverter->convertToMM(
                $background['left'],
                $this->pdf->getW()
            );
            $background['right'] = $this->cssConverter->convertToMM(
                $background['right'],
                $this->pdf->getW()
            );
            ...
            $res = false;
            $background['color'] = isset($param['backcolor'])
                ? $this->cssConverter->convertToColor($param['backcolor'], $res)
                : null;
            if (!$res) {
    ...
    protected function _tag_open_PAGE_HEADER($param)
    {
        ...
        for (
            $this->_parsePos;
            $this->_parsePos < $amountHtmlCodes;
            $this->_parsePos++
        ) {
            $action = $this->parsingHtml->code[$this->_parsePos];
            ...
            if (
                strtolower($action->getName()) === 'page_header_sub' &&
                $action->isClose()
            ) {
                break;
            }
    ...
    protected function _tag_open_PAGE_FOOTER($param)
    {
        ...
        for (
            ...
            if (
                strtolower($action->getName()) === 'page_footer_sub' &&
                $action->isClose()
            ) {
                break;
            }
    ...
    protected function _tag_open_PAGE_HEADER_SUB($param)
    {
        ...
        $this->parsingCss->value['width'] =
            $this->pdf->getW() - $this->_defaultLeft - $this->_defaultRight;
    ...
    protected function _tag_open_PAGE_FOOTER_SUB($param)
    {
        ...
        $sub = $this->createSubHTML();
        $sub->parsingHtml->code = $this->parsingHtml->getLevel(
            $this->_parsePos
        );
        $sub->_makeHTMLcode();
        $this->pdf->SetY(
            $this->pdf->getH() - $sub->_maxY - $this->_defaultBottom - 0.01
        );
        $this->_destroySubHTML($sub);
    ...
    protected function _tag_open_DIV($param, $other = 'div')
    {
        ...
        $marge['b'] =
            ...
            $this->parsingCss->value['padding']['b'] +
            0.03;
        ...
        if ($this->parsingCss->value['overflow'] === 'hidden') {
            ...
            $overH = null;
            $overflow = false;
            $this->parsingCss->value['width'] = max(
                $w,
                $this->parsingCss->value['width']
            );
            $this->parsingCss->value['height'] = max(
                $h,
                $this->parsingCss->value['height']
            );
        }
        ...
        if (!$this->parsingCss->value['position']) {
            ...
            if ($parentWidth > $w) {
                if ($alignObject === 'center') {
                    $this->pdf->SetX(
                        $this->pdf->GetX() + ($parentWidth - $w) * 0.5
                    );
        ...
        if (!$this->_subPart && !$this->_isSubPart) {
            ...
            switch ($this->parsingCss->value['vertical-align']) {
                ...
                case 'middle':
                    $yCorr =
                        ($this->parsingCss->value['height'] - $hReel) * 0.5;
                    break;
            }
        }
    ...
    protected function _tag_open_FIELDSET($param)
    {
        ...
        for (
            $tempPos = $this->_parsePos + 1;
            $tempPos < $amountHtmlCodes;
            $tempPos++
        ) {
            ...
            if ($action->getName() === 'legend' && !$action->isClose()) {
                ...
                for (
                    $sub->_parsePos = 0;
                    $sub->_parsePos < $amountSubHtmlCodes;
                    $sub->_parsePos++
                ) {
                    $action = $sub->parsingHtml->code[$sub->_parsePos];
    ...
    protected function _tag_close_DIV($param, $other = 'div')
    {
        ...
        if ($this->parsingCss->value['position'] !== 'absolute') {
            ...
            $this->pdf->SetXY(
                $this->parsingCss->value['xc'],
                $this->parsingCss->value['yc']
            );
        ...
        $newLineAfter =
            $this->parsingCss->value['display'] !== 'inline' &&
            $this->parsingCss->value['position'] !== 'absolute';
        $newPageAfter =
            $this->parsingCss->value['page-break-after'] == 'always';
    ...
    protected function _tag_open_BARCODE($param)
    {
        ...
        $txt =
            $param['label'] !== 'none'
                ? $this->parsingCss->value['font-size']
                : false;
        $c = $this->parsingCss->value['color'];
        $infos = $this->pdf->myBarcode(
            $param['value'],
            $param['type'],
            $x,
            $y,
    ...
    {
        $fill =
            $this->parsingCss->value['background']['color'] !== null &&
            $this->parsingCss->value['background']['image'] === null;
        if (
            in_array($this->parsingCss->value['id_tag'], [
                'fieldset',
                'legend',
                'div',
                'table',
        ...
        $txt = str_replace(
            '[[page_nb]]',
            $this->pdf->getMyAliasNbPages(),
            $txt
        );
        $txt = str_replace(
            '[[page_cu]]',
            $this->pdf->getMyNumPage($this->_page),
            $txt
        );
        ...
        if ($this->parsingCss->value['text-transform'] !== 'none') {
            if ($this->parsingCss->value['text-transform'] === 'capitalize') {
                ...
                $this->parsingCss->value['text-transform'] === 'uppercase'
            ) {
                $txt = mb_convert_case($txt, MB_CASE_UPPER, $this->_encoding);
            } elseif (
                $this->parsingCss->value['text-transform'] === 'lowercase'
            ) {
            ...
            if (count($words)) {
                ...
                if ($nb > $this->_sentenceMaxLines) {
                    ...
                    $e = new LongSentenceException(
                        'The current sentence takes more than ' .
                            $this->_sentenceMaxLines .
                            ' lines is the current box'
                    );
                    $e->setSentence($txt);
    ...
    protected function _tag_open_HR($param)
    {
        ...
        $this->parsingCss->save();
        $this->parsingCss->value['height'] = $this->cssConverter->convertToMM(
            '1mm'
        );
    ...
    protected function _tag_open_H1($param, $other = 'h1')
    {
        ...
        $this->parsingCss->value['margin'][
            't'
        ] = $this->cssConverter->convertToMM('16px');
        $this->parsingCss->value['margin'][
            'b'
        ] = $this->cssConverter->convertToMM('16px');
        $this->parsingCss->value[
            'font-size'
        ] = $this->cssConverter->convertFontSize($size[$other]);
    ...
    protected function _tag_open_P($param)
    {
        ...
        if (
            !in_array($this->_previousCall, ['_tag_close_P', '_tag_close_UL'])
        ) {
            if ($this->_maxH) {
                $this->_tag_open_BR([]);
            }
    ...
    protected function _tag_open_UL($param, $other = 'ul')
    {
        ...
        // add a level of list
        $start = isset($this->parsingCss->value['start'])
            ? $this->parsingCss->value['start']
            : null;
        $this->_listeAddLevel(
            $other,
    ...
    protected function _tag_open_LI($param)
    {
        ...
        // if small LI
        if ($inf[1]) {
            $this->parsingCss->value['mini-decal'] +=
                $this->parsingCss->value['mini-size'] * 0.045;
        ...
        if ($this->_subPart) {
            ...
            $this->parsingHtml->code[$tmpPos + 1] = new Node(
                $name,
                $params,
                false
            );
            $this->parsingHtml->code[$tmpPos + 2] = new Node(
                'li',
                $paramPUCE,
                true
            );
    ...
    protected function _tag_open_THEAD($param)
    {
        ...
        if ($this->_subPart) {
            self::$_tables[$param['num']]['thead']['tr'][0] =
                self::$_tables[$param['num']]['tr_curr'];
            self::$_tables[$param['num']]['thead']['code'] = [];
            ...
            for ($pos = $this->_tempPos; $pos < $amountHtmlCodes; $pos++) {
                ...
                if (
                    strtolower($action->getName()) === 'thead_sub' &&
                    $action->isClose()
                ) {
                    break;
                }
            ...
            self::$_tables[$param['num']]['tr_curr'] += count(
                self::$_tables[$param['num']]['thead']['tr']
            );
        }
    ...
    protected function _tag_open_TFOOT($param)
    {
        ...
        if ($this->_subPart) {
            self::$_tables[$param['num']]['tfoot']['tr'][0] =
                self::$_tables[$param['num']]['tr_curr'];
            self::$_tables[$param['num']]['tfoot']['code'] = [];
            ...
            for ($pos = $this->_tempPos; $pos < $amountHtmlCodes; $pos++) {
                ...
                if (
                    strtolower($action->getName()) === 'tfoot_sub' &&
                    $action->isClose()
                ) {
                    break;
                }
            ...
            $this->_parsePos += count($level);
            self::$_tables[$param['num']]['tr_curr'] += count(
                self::$_tables[$param['num']]['tfoot']['tr']
            );
        }
    ...
    protected function _tag_open_TABLE($param, $other = 'table')
    {
        ...
        $alignObject = isset($param['align'])
            ? strtolower($param['align'])
            : 'left';
        ...
        $collapse = false;
        if ($other === 'table') {
            $collapse = isset($this->parsingCss->value['border']['collapse'])
                ? $this->parsingCss->value['border']['collapse']
                : false;
        }
        ...
        if ($this->_subPart) {
            ...
            self::$_tables[$param['num']] = [];
            self::$_tables[$param['num']]['border'] = isset($param['border'])
                ? $this->parsingCss->readBorder($param['border'])
                : null;
            self::$_tables[$param['num']][
                'cellpadding'
            ] = $this->cssConverter->convertToMM(
            ...
            self::$_tables[$param['num']]['cases'] = []; // properties of each TR/TD
            self::$_tables[$param['num']]['corr'] = []; // link between TR/TD and colspan/rowspan
            ...
            self::$_tables[$param['num']]['marge'] = [];
            self::$_tables[$param['num']]['marge']['t'] =
                $this->parsingCss->value['padding']['t'] +
                $this->parsingCss->value['border']['t']['width'] +
                self::$_tables[$param['num']]['cellspacing'] * 0.5;
            self::$_tables[$param['num']]['marge']['r'] =
                $this->parsingCss->value['padding']['r'] +
                $this->parsingCss->value['border']['r']['width'] +
                self::$_tables[$param['num']]['cellspacing'] * 0.5;
            self::$_tables[$param['num']]['marge']['b'] =
                $this->parsingCss->value['padding']['b'] +
                $this->parsingCss->value['border']['b']['width'] +
                self::$_tables[$param['num']]['cellspacing'] * 0.5;
            self::$_tables[$param['num']]['marge']['l'] =
                $this->parsingCss->value['padding']['l'] +
                $this->parsingCss->value['border']['l']['width'] +
                self::$_tables[$param['num']]['cellspacing'] * 0.5;
            ...
            self::$_tables[$param['num']]['thead'] = []; // properties on the thead
            self::$_tables[$param['num']]['tfoot'] = []; // properties on the tfoot
            self::$_tables[$param['num']]['thead']['tr'] = []; // list of the TRs in the thead
            self::$_tables[$param['num']]['tfoot']['tr'] = []; // list of the TRs in the tfoot
            self::$_tables[$param['num']]['thead']['height'] = 0; // thead height
            self::$_tables[$param['num']]['tfoot']['height'] = 0; // tfoot height
            self::$_tables[$param['num']]['thead']['code'] = []; // HTML content of the thead
            self::$_tables[$param['num']]['tfoot']['code'] = []; // HTML content of the tfoot
            self::$_tables[$param['num']]['cols'] = []; // properties of the COLs
            ...
            $this->_drawRectangle(
                ...
                isset(self::$_tables[$param['num']]['height'][0])
                    ? self::$_tables[$param['num']]['height'][0]
                    : null,
                $this->parsingCss->value['border'],
                $this->parsingCss->value['padding'],
                0,
            ...
            self::$_tables[$param['num']]['style_value'] =
                $this->parsingCss->value;
        }
        ...
        if ($this->_subPart) {
            ...
            foreach ($lst as $mode) {
                ...
                foreach (self::$_tables[$param['num']][$mode]['tr'] as $tr) {
                    ...
                    for ($i = 0; $i < $nbTrs; $i++) {
                        if (
                            self::$_tables[$param['num']]['cases'][$tr][$i][
                                'rowspan'
                            ] == 1
                        ) {
                            $h = max(
            ...
            $h0 =
                ...
                self::$_tables[$param['num']]['marge']['b'];
            $h0 +=
                self::$_tables[$param['num']]['thead']['height'] +
                self::$_tables[$param['num']]['tfoot']['height'];
            ...
            for ($k = 0; $k < $nbCases; $k++) {
                ...
                if (
                    in_array($k, self::$_tables[$param['num']]['thead']['tr'])
                ) {
                    continue;
                }
                if (
                    in_array($k, self::$_tables[$param['num']]['tfoot']['tr'])
                ) {
                    continue;
                }
                ...
                for ($i = 0; $i < $nbCasesK; $i++) {
                    $h = max(
                        $h,
                        self::$_tables[$param['num']]['cases'][$k][$i]['h']
                    );

                    if (
                        self::$_tables[$param['num']]['cases'][$k][$i][
                            'rowspan'
                        ] == 1
                    ) {
                        $th = max(
            ...
            if (count(self::$_tables[$param['num']]['height']) > 1) {
                $y =
                    $this->_margeTop +
                    self::$_tables[$param['num']]['height'][
                        count(self::$_tables[$param['num']]['height']) - 1
                    ];
    ...
    protected function _tag_open_TR($param, $other = 'tr')
    {
        ...
        if (!$this->_subPart) {
            ...
            $nbTrCurrs = count(
                self::$_tables[$param['num']]['cases'][
                    self::$_tables[$param['num']]['tr_curr'] - 1
                ]
            );
            ...
            if (
                self::$_tables[$param['num']]['new_page'] &&
                count(self::$_tables[$param['num']]['thead']['code'])
            ) {
    ...
    protected function _tag_open_TD($param, $other = 'td')
    {
        ...
        $param['cellpadding'] =
            self::$_tables[$param['num']]['cellpadding'] . 'mm';
        $param['cellspacing'] =
            self::$_tables[$param['num']]['cellspacing'] . 'mm';
        ...
        if (in_array($other, ['td', 'th'])) {
            // id of the column
            $numCol = isset(
                self::$_tables[$param['num']]['cases'][$y][$x]['Xr']
            )
                ? self::$_tables[$param['num']]['cases'][$y][$x]['Xr']
                : self::$_tables[$param['num']]['corr_x'];
            ...
            if (isset(self::$_tables[$param['num']]['cols'][$numCol])) {
                ...
                for ($k = 0; $k < $colspan; $k++) {
                    if (
                        isset(
                            self::$_tables[$param['num']]['cols'][$numCol + $k][
                                'style'
                            ]['width']
                        )
                    ) {
        ...
        if ($specialLi) {
            $this->parsingCss->value[
                'width'
            ] -= $this->cssConverter->convertToMM($this->_listeGetWidth());
            $this->parsingCss->value[
                'width'
            ] -= $this->cssConverter->convertToMM($this->_listeGetPadding());
        }
        $this->parsingCss->setPosition();
        $this->parsingCss->fontSet();
        ...
        if ($collapse) {
            if (!$this->_subPart) {
                if (
                    ...
                    $this->parsingCss->value['border'][
                        't'
                    ] = $this->parsingCss->readBorder('none');
                }
            }
        ...
        $marge['t'] =
            $this->parsingCss->value['padding']['t'] +
            0.5 * self::$_tables[$param['num']]['cellspacing'] +
            $this->parsingCss->value['border']['t']['width'];
        $marge['r'] =
            $this->parsingCss->value['padding']['r'] +
            0.5 * self::$_tables[$param['num']]['cellspacing'] +
        ...
        if ($this->_subPart) {
            ...
            self::$_tables[$param['num']]['cases'][$y][$x] = [];
            ...
            self::$_tables[$param['num']]['cases'][$y][$x]['Xr'] =
                self::$_tables[$param['num']]['corr_x'];
            self::$_tables[$param['num']]['cases'][$y][$x]['Yr'] =
                self::$_tables[$param['num']]['corr_y'];
            ...
            for ($j = 0; $j < $rowspan; $j++) {
                for ($i = 0; $i < $colspan; $i++) {
                    ...
                    ][self::$_tables[$param['num']]['corr_x'] + $i] =
                        $i + $j > 0 ? '' : [$x, $y, $colspan, $rowspan];
                }
            }
            ...
            self::$_tables[$param['num']]['td_curr']++;
            self::$_tables[$param['num']]['td_x'] +=
                self::$_tables[$param['num']]['cases'][$y][$x]['dw'];
    ...
    protected function _tag_close_TD($param)
    {
        ...
        if ($this->_subPart) {
            ...
            if (
                $this->_testTdInOnepage &&
                $this->_subHtml->pdf->getPage() > 1
            ) {
                throw new TableException(
                    'The content of the TD tag does not fit on only one page'
                );
            }
            ...
            self::$_tables[$param['num']]['cases'][
                self::$_tables[$param['num']]['tr_curr'] - 1
            ][self::$_tables[$param['num']]['td_curr'] - 1]['w'] = max([
                $w0,
                $w2,
            ]);
            self::$_tables[$param['num']]['cases'][
                self::$_tables[$param['num']]['tr_curr'] - 1
            ][self::$_tables[$param['num']]['td_curr'] - 1]['h'] = max([
                $h0,
                $h2,
            ]);
            ...
            self::$_tables[$param['num']]['cases'][
                self::$_tables[$param['num']]['tr_curr'] - 1
            ][self::$_tables[$param['num']]['td_curr'] - 1]['real_w'] = $w0;
            self::$_tables[$param['num']]['cases'][
                self::$_tables[$param['num']]['tr_curr'] - 1
            ][self::$_tables[$param['num']]['td_curr'] - 1]['real_h'] = $h0;
            ...
            self::$_tables[$param['num']]['td_x'] +=
                self::$_tables[$param['num']]['cases'][
                    self::$_tables[$param['num']]['tr_curr'] - 1
                ][self::$_tables[$param['num']]['td_curr'] - 1]['w'];
        }
    ...
    protected function _tag_open_OPTION($param)
    {
        ...
        $value = isset($param['value'])
            ? $param['value']
            : 'aut_tag_open_opt_' . (count($this->_lstSelect) + 1);
    ...
    protected function _tag_open_INPUT($param)
    {
        ...
        switch ($param['type']) {
            case 'checkbox':
                ...
                $checked =
                    isset($param['checked']) && $param['checked'] === 'checked';
                $this->pdf->CheckBox(
                    $name,
                    $w,
                    $checked,
    ...
    protected function _tag_open_END_LAST_PAGE($param)
    {
        ...
        $this->pdf->SetY(
            $this->pdf->getH() - $this->pdf->getbMargin() - $height
        );
    }
