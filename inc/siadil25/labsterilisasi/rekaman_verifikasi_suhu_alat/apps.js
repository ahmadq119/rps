var app = new Vue({
	...
	methods: {
		...
		tambahData() {
			...
			if (
				!this.newData.tanggal ||
				!this.newData.nama_alat ||
				!this.newData.hasil_pengukuran ||
				!this.newData.nilai_set
			) {
			...
			axios
				.post("proses.php", {
					...
					nama_alat: this.newData.nama_alat,
					tanggal: this.newData.tanggal,
					hasil_pengukuran: this.newData.hasil_pengukuran,
					nilai_set: this.newData.nilai_set,
					idpeg: this.newData.idpeg,
					nama_pegawai: this.newData.nama_pegawai,
				})
				.then((response) => {
		...
		updateData() {
			...
			axios
				.post("proses.php", {
					...
					hasil_pengukuran: this.currentData.hasil_pengukuran,
					nilai_set: this.currentData.nilai_set,
		...
		resetForm() {
			const now = new Date();
			this.newData.tanggal = now.toISOString().split("T")[0];
			this.newData.nama_alat = "";
			this.newData.hasil_pengukuran = "";
			this.newData.nilai_set = "";
			this.newData.idpeg = "";
		...
		searchAlat(query) {
			...
			axios
				.post("proses.php", { request: "ambildataalat", query: query })
				.then((response) => {
					this.namaAlat = response.data;
					console.log("Data alat:", this.namaAlat); // Tambahkan log untuk memeriksa data
				})
				.catch((error) => console.log(error));
		},

		selectAlat(p1) {
			this.newData.nama_alat = p1.nama_alat;
		},

		selectAlatNew(p2) {
			this.currentData.nama_alat = p2.nama_alat;
		},
	...
	created() {
		...
		this.debouncedSearchAlat = this.debounce(this.searchAlat, 200);
	},
