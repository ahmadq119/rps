...
if ($request == 'tampil') {
    ...
    $sql = "SELECT TOP (20) * FROM view_rek_uv_alat Where ruangid=? ORDER BY tgl_kegiatan DESC";
...
if ($request == 'create') {
    ...
    $checkSql = "SELECT COUNT(*) AS count FROM rekaman_uv_alat   
                 WHERE nama_alat = ? AND tgl_kegiatan = ? AND ruangid = ?";
    $checkParams = array($nama_alat, $tgl_kegiatan, $ruangid);
    ...
    $sql = "INSERT INTO rekaman_uv_alat       
                      (tgl_kegiatan, nama_alat, pra_mulai, pra_selesai, pasca_mulai, pasca_selesai, idpeg, ruangid)       
                      VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $params = array($tgl_kegiatan, $nama_alat, $pra_mulai, $pra_selesai, $pasca_mulai, $pasca_selesai, $idpeg, $ruangid);
...
if ($request == 'update') {
    ...
    $sql = sqlsrv_query($koneksi, "UPDATE rekaman_uv_alat 
        SET tgl_kegiatan='$tgl_kegiatan', nama_alat='$nama_alat', pra_mulai='$pra_mulai', pra_selesai='$pra_selesai', pasca_mulai='$pasca_mulai', pasca_selesai='$pasca_selesai', idpeg='$idpeg', ruangid='$ruangid'
        WHERE id=$id");
...
if ($request == 'cari_seluruh_data') {
    $sql = sqlsrv_query($koneksi, "SELECT * FROM view_rek_uv_alat ORDER BY tgl_kegiatan DESC");
    $response = array();
...
if ($request == 'ambildataalat') {
    $query = $data->query;
    $sql = sqlsrv_query($koneksi, "SELECT DISTINCT nama_alat FROM rekaman_uv_alat WHERE nama_alat LIKE '%$query%' ORDER BY nama_alat ASC");
    $response = array();
    while ($row = sqlsrv_fetch_array($sql, SQLSRV_FETCH_ASSOC)) {
        $response[] = $row;
    }
