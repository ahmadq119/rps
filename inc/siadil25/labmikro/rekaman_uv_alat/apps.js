	...
	methods: {
		...
		tambahData() {
			...
			axios
				...
				.catch((error) => {
					alert(
						"Terjadi kesalahan saat menambahkan data. Silakan coba lagi. Mungkin data sudah ada sebelumnya"
					);
					console.log(error);
				});
		},
		...
		searchAlat(query) {
			...
			axios
				...
				.then((response) => {
					this.alat = response.data;
					console.log("Data alat:", this.alat); // Tambahkan log untuk memeriksa data
				})
				.catch((error) => console.log(error));
		},

		selectAlat(a1) {
			this.newData.nama_alat = a1.nama_alat;
		},

		selectAlatNew(a2) {
			this.currentData.nama_alat = a2.nama_alat;
		},
