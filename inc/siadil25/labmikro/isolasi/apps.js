var app = new Vue({
	...
	methods: {
		...
		validateForm(data) {
			const fields = ["kd_sample", "panjang", "berat"];
		...
		/*
		searchPetugasPreparasi(query) {
			if (query.length < 2) {
				this.petugasPreparasi = [];
				return;
			}
			axios
				.post("proses.php", { request: "ambildatapegawai", query: query })
				.then((response) => {
					this.petugasPreparasi = response.data;
				})
				.catch((error) => console.log(error));
		},
		selectPetugasPreparasi(p1) {
		...
		*/
		...
		getDataMetodeIsolasi() {
			axios
				.post("proses.php", { request: "ambildatametodeisolasi" })
				.then((response) => {
					if (response.data) {
						this.metodeIsolasi = response.data;
						console.log("Data Metode Isolasi:", this.metodeIsolasi);
					} else {
	...
	created() {
		this.debouncedSearchPetugasPreparasi = this.debounce(
			this.searchPetugasPreparasi,
			100
		);
	},
	mounted() {
		this.tampildata();
		this.getDataMetodeIsolasi();
		//this.getPetugasPreparasi();
