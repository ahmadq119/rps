var app = new Vue({
	...
	methods: {
		...
		validateForm(data) {
			const fields = ["kd_sample", "nama_bakteri_ditemukan"];
		...
		searchPenyelia(query) {
			if (query.length < 2) {
				this.petugasPenyelia = [];
				return;
			}
			axios
				.post("proses.php", { request: "ambildatapegawai", query: query })
				.then((response) => {
					this.petugasPenyelia = response.data;
				})
				.catch((error) => console.log(error));
		},
		selectPenyelia(p1) {
			this.currentData.penyelia = p1.nama_pegawai;
			this.currentData.idpeg_penyelia = p1.idpeg;
		},
		getPenyelia() {
			axios
				.post("proses.php", { request: "ambildatapegawai" })
	...
	created() {
		this.debouncedSearchAnalis = this.debounce(this.searchAnalis, 100);
		this.debouncedSearchPenyelia = this.debounce(this.searchPenyelia, 100);
	},
	mounted() {
		this.tampildata();
		this.getAnalis();
		this.getPenyelia();
		var editDataModalElement = document.getElementById("addDataModal");
