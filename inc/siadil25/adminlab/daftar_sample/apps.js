var app = new Vue({
	...
	methods: {
		...
		navigateToDisposisi(kd_sample) {
			...
			window.location.href = `../disposisi/index.php?kd_sample=${kd_sample}`;
		},
		...
		printDisposisi(data) {
			...
			const nama_sample = data.nama_sample;
			const no_reg = data.no_reg;
			const tgl_terima = data.tgl_terima;
			const mt = data.mt;

			// Construct the URL for the PHP script
			const url = `print_disposisi.php?kd_sample=${kd_sample}&nama_sample=${nama_sample}&jumlah=${jumlah}&satuan=${satuan}&no_reg=${no_reg}&tgl_terima=${tgl_terima}&mt=${mt}`;
