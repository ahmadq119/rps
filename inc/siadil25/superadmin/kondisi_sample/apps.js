var app = new Vue({
	...
	methods: {
		...
		tambahData() {
			...
			/*
			if (!this.newData.kondisi_sample) {
			...
			*/
			axios
				.post("proses.php", {
					request: "create",
					kondisi_sample: this.newData.kondisi_sample,
				})
				.then((response) => {
		...
		updateData() {
			...
			axios
				.post("proses.php", {
					...
					kondisi_sample: this.currentData.kondisi_sample,
				})
				.then((response) => {
					let index = this.info.findIndex(
						(item) => item.idkondisi === this.currentData.idkondisi
					);
					if (index !== -1) {
