var app = new Vue({
	...
	methods: {
		logout() {
			axios
				.post("../Login/proses.php", { request: "logout" })
				.then((response) => {
		...
		hapus(index, idpeg) {
			axios
				.post("proses.php", { request: "delete", idpeg: idpeg })
				.then(() => this.info.splice(index, 1))
				.catch((error) => console.log(error));
		},
		confirmHapus(index, idpeg) {
			if (confirm("Apakah record ini akan dihapus?")) {
				this.hapus(index, idpeg);
			}
		},
		caridata() {
			axios
		...
		tambahData() {
			...
			axios
				.post("proses.php", {
					...
					nip_pegawai: this.newData.nip_pegawai,
				})
				.then((response) => {
			...
			axios
				...
				.then((response) => {
					let index = this.info.findIndex(
						(item) => item.idpeg === this.currentData.idpeg
					);
					if (index !== -1) {
		...
		validateForm(data) {
			const fields = ["nama_pegawai", "nip_pegawai"];
